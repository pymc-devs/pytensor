...
from pytensor.tensor.subtensor import AdvancedIncSubtensor, AdvancedSubtensor1, Subtensor
...
class TestDots(utt.InferShapeTester):
    def setup_method(self):
        ...
        self.v_10 = np.asarray(np.random.uniform(-1, 1, 10), dtype=pytensor.config.floatX)
...
class TestUsmm:
    ...
    def setup_method(self):
        ...
        self.y = np.asarray(self.rng.uniform(-1, 1, y_size), dtype=pytensor.config.floatX)
        self.z = np.asarray(self.rng.uniform(-1, 1, z_size), dtype=pytensor.config.floatX)
...
def test_sparse_shared_memory():
    ...
    f = pytensor.function([In(x, mutable=True), In(y, mutable=True)], z, mode="FAST_RUN")
