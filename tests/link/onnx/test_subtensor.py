...
from tests.link.onnx.strategies import INCSUBTENSOR_OPERATIONS, SUBTENSOR_OPERATIONS
...
def test_subtensor_basic_slicing_correctness(op_name, data):
    ...
    Total: 3 patterns x 20 examples = 60 test scenarios
    ...
    assert result.ndim <= x_val.ndim, (
        "Result should not have more dimensions than input"
    )
    assert result.size <= x_val.size, "Slice result should not be larger than input"
...
def test_advanced_subtensor_indexing_correctness(data):
    ...
    assert np.all(indices_val < x_val.shape[0]), "Indices should be within bounds"
...
def test_inc_subtensor_operation_correctness(data):
    ...
    assert "Add" in node_types, "Expected Add operation (for increment)"
...
class TestIncSubtensor:
    ...
    def test_set_subtensor(self):
        ...
        node_types = get_onnx_node_types(fn)
        assert "ScatterElements" in node_types, (
            f"Expected 'ScatterElements' in {node_types}"
        )
