...
def test_alloc_empty_shape_dtype():
    ...
    fn, result = compare_onnx_and_py(
        [], x, [],
        assert_fn=lambda a, b: (
            a.shape == b.shape and a.dtype == b.dtype
        ) or (_ for _ in ()).throw(AssertionError(f"Shape/dtype mismatch: {a.shape}/{a.dtype} vs {b.shape}/{b.dtype}"))
    )
...
def test_arange_with_different_dtypes():
    ...
    fn_int, result_int = compare_onnx_and_py([], x_int, [])
    ...
    fn_float, result_float = compare_onnx_and_py([], x_float, [])
