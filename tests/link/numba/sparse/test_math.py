...
def test_structured_dot_grad(x_format, y_format, x_shape, y_shape):
    ...
    x_test = scipy.sparse.random(
        *x_shape, density=0.4, format=x_format, random_state=rng
    )
    ...
    if y_format is None:
        ...
        g_xy_test = scipy.sparse.random(
            *g_xy_shape, density=0.3, format=x_format, random_state=rng
        )
...
def test_sparse_sum(format, axis):
    rng = np.random.default_rng(
        sum([156, format == "csr", -1 if axis is None else axis])
    )

    x = ps.matrix(format=format, name="x", shape=(7, 5))
    z = ps.sp_sum(x, axis=axis)
    x_test = scipy.sparse.random(7, 5, density=0.4, format=format, random_state=rng)
