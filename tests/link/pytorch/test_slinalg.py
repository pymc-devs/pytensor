...
def test_pytorch_basic():
    ...
    _, [pytorch_res] = compare_pytorch_and_py(out_fg, test_input_vals)

    # Confirm that the `Subtensor` slice operations are correct
    assert pytorch_res.shape == (5, 3)

    # Confirm that the `IncSubtensor` operations are correct
    assert pytorch_res[0, 0] == -10.0
    assert pytorch_res[0, 1] == -8.0
    ...
    compare_pytorch_and_py(out_fg, [np.arange(10).astype(config.floatX)])
