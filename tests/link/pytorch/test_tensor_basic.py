...
def test_pytorch_Alloc():
    x = at.alloc(0.0, 2, 3)
    x_fg = FunctionGraph([], [x])

    _, [pytorch_res] = compare_pytorch_and_py(x_fg, [])

    assert pytorch_res.shape == (2, 3)
    ...
    compare_pytorch_and_py(x_fg, [], assert_fn=compare_shape_dtype)

    a = scalar("a")
    x = at.alloc(a, 20)
    x_fg = FunctionGraph([a], [x])

    compare_pytorch_and_py(x_fg, [10.0])

    a = vector("a")
    x = at.alloc(a, 20, 10)
    x_fg = FunctionGraph([a], [x])

    compare_pytorch_and_py(x_fg, [np.ones(10, dtype=config.floatX)])


def test_alloc_runtime_broadcast():
    TestAlloc.check_runtime_broadcast(get_mode("PyTorch"))
...
def test_arange_of_shape():
    ...
    compare_pytorch_and_py(fgraph, [np.zeros((5,))])


def test_arange_nonconcrete():
    """PyTorch cannot JIT-compile `pytorch.numpy.arange` when arguments are not concrete values."""
...
class TestJaxSplit:
    ...
    def test_runtime_errors(self):
        ...
        fn = pytensor.function([a], a_splits, mode="PyTorch")
        with pytest.raises(
    ...
    def test_pytorch_split_not_supported(self):
        a = matrix("a", shape=(6, None))
...
def test_pytorch_eye():
    """Tests pytorchification of the Eye operator"""
    out = at.eye(3)
...
def test_tri_nonconcrete():
    """PyTorch cannot JIT-compile `pytorch.numpy.tri` when arguments are not concrete values."""
    ...
    # The actual error the user will see should be pytorch.errors.ConcretizationTypeError, but
