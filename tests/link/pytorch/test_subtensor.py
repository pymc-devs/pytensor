...
def test_pytorch_Subtensor_constant():
    # Basic indices
...
def test_pytorch_Subtensor_dynamic():
    a = at.iscalar("a")
    x = at.arange(3)
    out_at = x[:a]
    assert isinstance(out_at.owner.op, at_subtensor.Subtensor)
    out_fg = FunctionGraph([a], [out_at])
    compare_pytorch_and_py(out_fg, [1])


def test_pytorch_Subtensor_boolean_mask():
    """PyTorch does not support resizing arrays with boolean masks."""
    ...
    with pytest.raises(NotImplementedError, match="resizing arrays with boolean"):
        compare_pytorch_and_py(out_fg, [x_at_test])


def test_pytorch_Subtensor_boolean_mask_reexpressible():
    ...
    `pytorch_boolean_indexing_sum` rewrite.

    PyTorch forces users to re-express this logic manually, so this is an
...
def test_pytorch_IncSubtensor_boolean_indexing_reexpressible():
