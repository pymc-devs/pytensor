...
def test_scan_sit_sot(view):
    ...
    compare_pytorch_and_py(fg, test_input_vals)
...
def test_scan_nit_sot(view):
    ...
    pytorch_fn, _ = compare_pytorch_and_py(
        fg, test_input_vals, pytorch_mode=get_mode("PyTorch").excluding("scan_pushout")
    )
    scan_nodes = [
        node for node in pytorch_fn.maker.fgraph.apply_nodes if isinstance(node.op, Scan)
    ]
...
def test_scan_mit_mot():
    ...
    compare_pytorch_and_py(fg, [np.arange(10)])
...
def test_scan_update():
    ...
    pytorch_fn = function([], xs, updates=update, mode="PyTorch")
    np.testing.assert_array_equal(pytorch_fn(), np.array([1, 2, 4, 8, 16, 32, 64]) + 0.0)

    sh_static.set_value(1.0)
    np.testing.assert_array_equal(
        pytorch_fn(), np.array([128, 256, 512, 1024, 2048, 4096, 8192]) + 1.0
    )

    sh_static.set_value(2.0)
    sh_update.set_value(1.0)
    np.testing.assert_array_equal(pytorch_fn(), np.array([1, 2, 4, 8, 16, 32, 64]) + 2.0)
...
def test_scan_rng_update():
    ...
    with pytest.warns(
        UserWarning,
        match=re.escape("[rng] will not be used in the compiled PyTorch graph"),
    ):
        pytorch_fn = function([], [xs], updates=None, mode="PyTorch")

    res1, res2 = pytorch_fn(), pytorch_fn()
...
def test_scan_SEIR():
    ...
    # TODO: Use random streams when their PyTorch conversions are implemented.
    ...
    compare_pytorch_and_py(out_fg, test_input_vals)
...
def test_nd_scan_sit_sot(x0_func, A_func):
    ...
    # Must specify mode = PyTorch for the inner func to avoid a GEMM Op in the PyTorch graph
    xs, _ = scan(
        lambda X, A: A @ X,
        non_sequences=[A],
        outputs_info=[x0],
        n_steps=n_steps,
        mode=get_mode("PyTorch"),
    )
...
def test_default_mode_excludes_incompatible_rewrites():
    ...
    compare_pytorch_and_py(fg, [np.eye(3), np.eye(3)])
