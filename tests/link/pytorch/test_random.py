...
from tests.link.pytorch.test_basic import compare_pytorch_and_py, pytorch_mode, set_test_value
...
def test_random_RandomStream():
    ...
    fn = random_function([], out, mode=pytorch_mode)
    pytorch_res_1 = fn()
    pytorch_res_2 = fn()

    assert not np.array_equal(pytorch_res_1, pytorch_res_2)
...
def test_random_updates(rng_ctor):
    ...
    f = random_function([], [x], updates={rng: next_rng}, mode=pytorch_mode)
    assert f() != f()

    # Check that original rng variable content was not overwritten when calling pytorch_typify
    assert all(
...
def test_random_RandomVariable(rv_op, dist_params, base_size, cdf_name, params_conv):
    """The PyTorch samplers are not one-to-one with NumPy samplers so we
    ...
    g_fn = random_function(dist_params, g, mode=pytorch_mode)
    samples = g_fn(
        *[
            i.tag.test_value
...
def test_random_bernoulli(size):
    ...
    g_fn = random_function([], g, mode=pytorch_mode)
    samples = g_fn()
...
def test_random_unimplemented():
    ...
    with pytest.raises(NotImplementedError):
        with pytest.warns(
            ...
            compare_pytorch_and_py(fgraph, [])


def test_random_custom_implementation():
    """We can register a PyTorch implementation for user-defined `RandomVariable`s"""
    ...
    from pytensor.link.pytorch.dispatch.random import pytorch_sample_fn

    @pytorch_sample_fn.register(CustomRV)
    def pytorch_sample_fn_custom(op):
...
def test_random_concrete_shape():
    """PyTorch should compile when a `RandomVariable` is passed a concrete shape.

    There are three quantities that PyTorch considers as concrete:
    ...
    pytorch_fn = random_function([x_at], out, mode=pytorch_mode)
    assert pytorch_fn(np.ones((2, 3))).shape == (2, 3)
...
def test_random_concrete_shape_subtensor():
    """PyTorch should compile when a concrete value is passed for the `size` parameter.
    ...
    inputs into tuples of concrete values using the `pytorch_size_parameter_as_tuple`
    rewrite.

    PyTorch does not accept scalars as `size` or `shape` arguments, so this is a
    ...
    pytorch_fn = random_function([x_at], out, mode=pytorch_mode)
    assert pytorch_fn(np.ones((2, 3))).shape == (3,)


def test_random_concrete_shape_subtensor_tuple():
    """PyTorch should compile when a tuple of concrete values is passed for the `size` parameter.
    ...
    `pytorch_size_parameter_as_tuple` rewrite.
    ...
    pytorch_fn = random_function([x_at], out, mode=pytorch_mode)
    assert pytorch_fn(np.ones((2, 3))).shape == (2,)
...
def test_random_concrete_shape_graph_input():
    ...
    pytorch_fn = random_function([size_at], out, mode=pytorch_mode)
    assert pytorch_fn(10).shape == (10,)
