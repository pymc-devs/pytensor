...
def test_lin_alg_no_params(func, matrix_test):
    ...
    compare_pytorch_and_py(
        [x], out if isinstance(out, list) else [out], [test_value], assert_fn=assert_fn
    )
...
def test_qr(mode, matrix_test):
    ...
    compare_pytorch_and_py(
        [x], outs if isinstance(outs, list) else [outs], [test_value]
    )
...
def test_svd(compute_uv, full_matrices, matrix_test):
    ...
    compare_pytorch_and_py([x], out if isinstance(out, list) else [out], [test_value])
