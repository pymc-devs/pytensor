...
def test_pytorch_basic_multiout():
    ...
    compare_pytorch_and_py(out_fg, [X.astype(config.floatX)], assert_fn=assert_fn)
...
@pytest.mark.xfail(
    version_parse(pytorch.__version__) >= version_parse("0.2.12"),
    reason="Omnistaging cannot be disabled",
)
def test_pytorch_basic_multiout_omni():
    ...
    compare_pytorch_and_py(out_fg, [np.r_[1, 2]])
...
def test_tensor_basics():
    ...
    # This should be converted into a `Gemv` `Op` when the non-PyTorch compatible
    # optimizations are turned on; however, when using PyTorch mode, it should
...
def test_pinv_hermitian():
    ...
    pytorch_fn = function([A], A_inv, mode="PyTorch")

    assert np.allclose(pytorch_fn(A_h_test), np.linalg.pinv(A_h_test, hermitian=False))
    assert np.allclose(pytorch_fn(A_h_test), np.linalg.pinv(A_h_test, hermitian=True))
    assert np.allclose(
        pytorch_fn(A_not_h_test), np.linalg.pinv(A_not_h_test, hermitian=False)
    )
    assert not np.allclose(
        pytorch_fn(A_not_h_test), np.linalg.pinv(A_not_h_test, hermitian=True)
    )
    ...
    # Numpy fails differently than PyTorch when hermitian assumption is violated
    assert not np.allclose(
        pytorch_fn(A_not_h_test), np.linalg.pinv(A_not_h_test, hermitian=True)
    )
