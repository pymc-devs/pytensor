...
def test_elemwise_runtime_broadcast():
    TestElemwise.check_runtime_broadcast(get_mode("PyTorch"))
...
def test_pytorch_CAReduce():
    ...
    compare_pytorch_and_py(x_fg, [np.r_[1, 2, 3].astype(config.floatX)])
    ...
    compare_pytorch_and_py(x_fg, [np.c_[[1, 2, 3], [1, 2, 3]].astype(config.floatX)])
...
def test_logsumexp_benchmark(size, axis, benchmark):
    ...
    X_lse_fn = pytensor.function([X], X_lse, mode="PyTorch")
...
def test_multiple_input_multiply():
    ...
    compare_pytorch_and_py(fg, [[1.5], [2.5], [3.5]])
