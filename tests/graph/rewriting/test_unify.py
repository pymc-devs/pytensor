...
def test_match_commutative_cross_node_conflict():
    # The valid assignment inside the commutative `add` (x=b, y=a) conflicts with the
    # positional one (x=a, y=b) only through the sibling "y". Deterministic children
    # are matched first so the conflict surfaces while `add` can still backtrack.
    a = pt.vector("a")
    b = pt.vector("b")
    pat = convert_strs_to_vars((pt.sub, (pt.add, "x", "y"), "y"))
    subs = match_pattern(pat, pt.sub(pt.add(a, b), a).owner)
    assert subs is not None
    by_name = {k.token: v for k, v in subs.items()}
    assert by_name["x"] is b
    assert by_name["y"] is a
