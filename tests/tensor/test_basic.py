...
class TestGetUnderlyingScalarConstant:
    def test_basic(self):
        with pytest.raises(NotScalarConstantError):
            get_underlying_scalar_constant(aes.int64())

        res = get_underlying_scalar_constant(at.as_tensor(10))
        assert res == 10
        assert isinstance(res, np.ndarray)

        res = get_underlying_scalar_constant(np.array(10))
        assert res == 10
        assert isinstance(res, np.ndarray)

        a = at.stack([1, 2, 3])
        assert get_underlying_scalar_constant(a[0]) == 1
        assert get_underlying_scalar_constant(a[1]) == 2
        assert get_underlying_scalar_constant(a[2]) == 3

        b = iscalar()
        a = at.stack([b, 2, 3])
        with pytest.raises(NotScalarConstantError):
            get_underlying_scalar_constant(a[0])
        assert get_underlying_scalar_constant(a[1]) == 2
        assert get_underlying_scalar_constant(a[2]) == 3

        # For now get_underlying_scalar_constant goes through only MakeVector and Join of
        # scalars.
        v = ivector()
        a = at.stack([v, [2], [3]])
        with pytest.raises(NotScalarConstantError):
            get_underlying_scalar_constant(a[0])
        with pytest.raises(NotScalarConstantError):
            get_underlying_scalar_constant(a[1])
        with pytest.raises(NotScalarConstantError):
            get_underlying_scalar_constant(a[2])

        # Test the case SubTensor(Shape(v)) when the dimensions
        # is broadcastable.
        v = row()
        assert get_underlying_scalar_constant(v.shape[0]) == 1

        res = at.get_underlying_scalar_constant(at.as_tensor([10, 20]).shape[0])
        assert isinstance(res, np.ndarray)
        assert 2 == res

        res = at.get_underlying_scalar_constant(
    ...
    def test_DimShufle(self):
        ...
        assert get_underlying_scalar_constant(a) == 1

    def test_subtensor_of_constant(self):
        c = constant(random(5))
        for i in range(c.value.shape[0]):
            assert get_underlying_scalar_constant(c[i]) == c.value[i]
        c = constant(random(5, 5))
        for i in range(c.value.shape[0]):
            for j in range(c.value.shape[1]):
                assert get_underlying_scalar_constant(c[i, j]) == c.value[i, j]

    def test_numpy_array(self):
        # Regression test for crash when called on a numpy array.
        assert get_underlying_scalar_constant(np.array(3)) == 3
        with pytest.raises(NotScalarConstantError):
            get_underlying_scalar_constant(np.array([0, 1]))
        with pytest.raises(NotScalarConstantError):
            get_underlying_scalar_constant(np.array([]))

    def test_make_vector(self):
        mv = make_vector(1, 2, 3)
        with pytest.raises(NotScalarConstantError):
            get_underlying_scalar_constant(mv)
        assert get_underlying_scalar_constant(mv[0]) == 1
        assert get_underlying_scalar_constant(mv[1]) == 2
        assert get_underlying_scalar_constant(mv[2]) == 3
        assert get_underlying_scalar_constant(mv[np.int32(0)]) == 1
        assert get_underlying_scalar_constant(mv[np.int64(1)]) == 2
        assert get_underlying_scalar_constant(mv[np.uint(2)]) == 3
        t = aes.ScalarType("int64")
        with pytest.raises(NotScalarConstantError):
            get_underlying_scalar_constant(mv[t()])

    def test_shape_i(self):
        c = constant(np.random.random((3, 4)))
        s = Shape_i(0)(c)
        assert get_underlying_scalar_constant(s) == 3
        s = Shape_i(1)(c)
        assert get_underlying_scalar_constant(s) == 4
        d = pytensor.shared(np.random.standard_normal((1, 1)), shape=(1, 1))
        f = ScalarFromTensor()(Shape_i(0)(d))
        assert get_underlying_scalar_constant(f) == 1
    ...
    def test_elemwise(self):
        ...
        s = c + 1
        assert np.allclose(get_underlying_scalar_constant(s), c.data + 1)
        s = c - 1
        assert np.allclose(get_underlying_scalar_constant(s), c.data - 1)
        s = c * 1.2
        assert np.allclose(get_underlying_scalar_constant(s), c.data * 1.2)
        s = c < 0.5
        assert np.allclose(get_underlying_scalar_constant(s), int(c.data < 0.5))
        s = at.second(c, 0.4)
        assert np.allclose(get_underlying_scalar_constant(s), 0.4)
    ...
    def test_assert(self):
        ...
        a = Assert()(c, c > 1)
        assert get_underlying_scalar_constant(a) == 2

        with config.change_flags(compute_test_value="off"):
            # condition is always False
            a = Assert()(c, c > 2)
            with pytest.raises(NotScalarConstantError):
                get_underlying_scalar_constant(a)
    ...
    def test_second(self):
        ...
        shp = vector()
        s = at.second(shp, c)
        assert get_underlying_scalar_constant(s) == c.data
