...
def test_convolve1d_valid_grad(static_shape):
    ...
    is_full_mode = conv_node.inputs[-1]
    ...
    if static_shape:
        assert is_full_mode.eval() == False  # noqa: E712
    else:
        dtype = larger.dtype
        larger_test = np.zeros((128,), dtype=dtype)
        smaller_test = np.zeros((64,), dtype=dtype)
        assert is_full_mode.eval({larger: larger_test, smaller: smaller_test}) == False  # noqa: E712
        assert is_full_mode.eval({larger: smaller_test, smaller: larger_test}) == True  # noqa: E712


def convolve1d_grad_benchmarker(convolve_mode, mode, benchmark):
    larger = tensor("larger", shape=(8, 1024))
    smaller = tensor("smaller", shape=(8, 16))
