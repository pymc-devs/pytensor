...
class TestLocalSumProd:
    ...
    def test_local_sum_sum_prod_prod(self):
        ...
        for d, dd in dims:
            ...
            np.testing.assert_allclose(f(input), expected, atol=1e-08, rtol=1e-05)
            assert len(f.maker.fgraph.apply_nodes) == 1
        for d, dd in dims[:6]:
            f = function([a], a.sum(d).sum(dd).sum(0), mode=self.mode)
            np.testing.assert_allclose(
                f(input), input.sum(d).sum(dd).sum(0), atol=1e-08, rtol=1e-05
            )
            assert len(f.maker.fgraph.apply_nodes) == 1
        for d in [0, 1, 2]:
            f = function([a], a.sum(d).sum(None), mode=self.mode)
            np.testing.assert_allclose(
                f(input), input.sum(d).sum(), atol=1e-08, rtol=1e-05
            )
            assert len(f.maker.fgraph.apply_nodes) == 1
        ...
        for d, dd in dims[:6]:
            ...
            np.testing.assert_allclose(
                f(input), input.prod(d).prod(dd).prod(0), atol=1e-08, rtol=1e-05
            )
            assert len(f.maker.fgraph.apply_nodes) == 1
        for d in [0, 1, 2]:
            f = function([a], a.prod(d).prod(None), mode=self.mode)
            np.testing.assert_allclose(
                f(input), input.prod(d).prod(), atol=1e-08, rtol=1e-05
            )
            assert len(f.maker.fgraph.apply_nodes) == 1
        f = function([a], a.prod(None).prod(), mode=self.mode)
        np.testing.assert_allclose(f(input), input.prod(), atol=1e-08, rtol=1e-05)
        assert len(f.maker.fgraph.apply_nodes) == 1
        ...
        for d, dd in dims[:6]:
            ...
            np.testing.assert_allclose(
                f(input), input.sum(d).prod(), atol=1e-08, rtol=1e-05
            )
            assert len(f.maker.fgraph.apply_nodes) == 2
        for d in [0, 1, 2]:
