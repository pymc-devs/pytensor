...
and :func:`pytensor.gradient.Rop`.

.. function:: pull_back(inputs, outputs, cotangents)
  ...
  the pullback computes :math:`\bar{x} = \bar{y}^T J` where
  ...
  language. The :meth:`Op.pull_back` method must return a list containing one
  ...
  the computation of only disconnected outputs, then :meth:`Op.pull_back` should
  return :class:`DisconnectedType` variables for those inputs.

  If :meth:`Op.pull_back` is not defined, then PyTensor assumes it has been
  ...
  vector-valued input ``x``, then :meth:`Op.pull_back` will be passed ``x``,
  ``y``, and a cotangent vector ``z``. Define ``J`` to be the Jacobian of ``y``
  with respect to ``x``. The method should return ``dot(J.T, z)``. When
  :func:`pytensor.grad` calls :meth:`Op.pull_back`, it will set ``z`` to be the
  gradient of the cost ``C`` with respect to ``y``. If this :class:`Op` is the only
  :class:`Op` that acts on ``x``, then ``dot(J.T, z)`` is the gradient of C with
  ...
  integer as an argument to your :meth:`Op.pull_back` method, recall the definition of
  ...
  1) :math:`f(x,y)` is a dot product between :math:`x` and :math:`y`. :math:`x` and :math:`y` are integers.
     ...
     Its gradient is zero almost everywhere, so :meth:`Op.pull_back` should return
  ...
  4) :math:`f(x,y)` is a vector with :math:`y` elements, each of which taking on
     the value :math:`x` The :meth:`Op.pull_back` method should return
...
.. function:: push_forward(inputs, outputs, tangents)
