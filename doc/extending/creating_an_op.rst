...
However, if you cannot implement an :ref:`op` in terms of an existing :ref:`op`, you have to write a new one.

This page will show how to implement some simple Python-based :ref:`op` that perform operations on numpy arrays.
...
details how to write such an :ref:`op` instance. Please refer to
:ref:`graphstructures` for a more detailed explanation about the graph
structure.


:ref:`op`'s basic methods
---------------------------

An :ref:`op` is any Python object that inherits from :ref:`op`.
This section provides an overview of the basic methods you typically have to
implement to make a new :ref:`op`.  It does not provide extensive coverage of all the
...
An :ref:`op` has to implement some methods defined in the the interface of
:ref:`op`. More specifically, it is mandatory for an :ref:`op` to define either
...
- Checks that the inputs can be converted to :ref:`variable`\s whose types are compatible with the current :ref:`op`.
...
- Returns an :ref:`apply` instance with the input and output :ref:`variable`\s, and itself as the :ref:`op`.

If :meth:`make_node` is not defined, the :attr:`itypes` and :attr:`otypes` are used by the :ref:`op`'s
...
:meth:`perform` method defines the Python implementation of an :ref:`op`.
...
- ``node`` is a reference to an :ref:`apply` node which was previously
  ...
  used in a simple :ref:`op`, but it contains symbolic information that
  could be required by a complex :ref:`op`.
...
- ``output_storage`` is a list of storage cells where the output
  is to be stored. There is one storage cell for each output of the :ref:`op`.
...
:ref:`op`'s auxiliary methods
-------------------------------

There are other methods that can be optionally defined by the :ref:`op`:

:attr:`__props__`
^^^^^^^^^^^^^^^^^^^^

The :attr:`__props__` attribute lists the :ref:`op` instance properties
...
According to this default, :meth:`__eq__`, two :ref:`op`\s will be equal if they have the same values for all
the properties listed in :attr:`__props__`. Similarly, they will have the same hash.

When PyTensor sees two nodes with equal :ref:`op`\s and the same set of inputs,
...
:attr:`__props__` will also generate a  suitable :meth:`__repr__` and :meth:`__str__` for your :ref:`op`.


:meth:`infer_shape`
^^^^^^^^^^^^^^^^^^^^^^

The :meth:`infer_shape` method allows an :ref:`op` to infer the shape of its
output variables without actually computing them.
It takes as input ``fgraph``, a :class:`FunctionGraph`; ``node``, a reference
to the :ref:`op`'s :ref:`apply` node;
and a list of :class:`Variables`\s (e.g. ``i0_shape``, ``i1_shape``, ...)
which are the dimensions of the :ref:`op` input :ref:`variable`\s.
...
whose expression includes your :ref:`op`. The gradient is
...
work with your :ref:`op`.

This function implements the application of the R-operator on the
function represented by your :ref:`op`. Let's assume that function is :math:`f`,
...
As you'll see below, you can then pass an instantiated :ref:`variable`, such as ``x = tensor.matrix("x")`` to the instantiated :ref:`op`,
to define a new :ref:`variable` that represents the output of applying the :ref:`op` to the input variable.
...
The :meth:`perform` method implements the :ref:`op`'s mathematical logic in Python.
...
but it must not influence the semantics of the :ref:`op` output.

You can try the new :ref:`op` as follows:
...
We create an :ref:`op` that takes a variable ``x`` and returns ``a*x+b``.
We want to say that two such :ref:`op`\s are equal when their values of ``a`` and ``b`` are equal.
...
Example: More complex :ref:`op`
---------------------------------

As a final example, we will create a multi-output :ref:`op` that takes a matrix and a vector and returns the matrix transposed and the sum of the vector.

Furthermore, this :ref:`op` will work with batched dimensions, meaning we can pass in a 3D tensor or a 2D tensor (or more) and it will work as expected.
...
PyTensor has some functionalities to test for a correct implementation of an :ref:`op` and it's many methods.

We have already seen some user facing helpers, but there are also test classes for :ref:`op` implementations
...
Basic tests are done by you just by using the :ref:`op` and checking that it returns the right answer.
...
It tests that the :ref:`op` gets rewritten out of the graph if only the shape of the output is needed and not the output itself.
...
It also takes the :ref:`op` class as a parameter in order to verify that no instance of it appears in the shape-optimized graph.
...
If your :ref:`op` works only with such matrices, you can disable the warning with the ``warn=False`` parameter.
...
As shown above, the function :ref:`verify_grad <validating_grad>` verifies the gradient of an :ref:`op` or PyTensor graph.
...
These allow to test the implementation of the :meth:`R_op` method of a particular :ref:`op`.
...
.. testcode:: tests
   ...
   class TestDoubleRop(RopLopChecker):
...
basic PyTensor :ref:`op` that will call the supplied function during execution.

This isn't the recommended way to build an :ref:`op`, but allows for a quick implementation.
...
.. warning::
    ...
    Not providing a :meth:`infer_shape` prevents shape-related rewrites from working with this :ref:`op`.
    For example ``your_op(inputs, ...).shape`` will need the :ref:`op` to be executed just to get the shape.

.. note::

    As no L_op is defined, this means you won't be able to
    differentiate paths that include this :ref:`op`.
...
.. note::
    ...
    an :ref:`op` that returns a view or modifies the values in its inputs. Thus, all
    :ref:`op`\s created with the instructions described here MUST return newly
    ...
    If your :ref:`op` returns a view or changes the value of its inputs
    ...
    can help verify whether or not your :ref:`op` behaves correctly in this
    regard.