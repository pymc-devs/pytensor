...
@pytorch_funcify.register(Scan)
def pytorch_funcify_Scan(op: Scan, **kwargs):
    info = op.info

    if info.as_while:
        raise NotImplementedError("While Scan cannot yet be converted to PyTorch")

    if info.n_mit_mot:
        raise NotImplementedError(
            "Scan with MIT-MOT (gradients of scan) cannot yet be converted to PyTorch"
        )
    ...
    def scan(*outer_inputs):
        # Extract PyTorch scan inputs
        outer_inputs = list(outer_inputs)
        n_steps = outer_inputs[0]  # PyTorch `length`
        seqs = op.outer_seqs(outer_inputs)  # PyTorch `xs`
        ...
        def pytorch_args_to_inner_func_args(carry, x):
            """Convert PyTorch scan arguments into format expected by scan_inner_func.
            ...
            for array, index in zip(inner_mit_sot, op.info.mit_sot_in_slices):
                mit_sot_flatten.extend(array[torch.tensor(index)])
        ...
        def inner_func_outs_to_pytorch_outs(
            old_carry,
            inner_scan_outs,
        ):
            """Convert inner_scan_func outputs into format expected by PyTorch scan.
            ...
            inner_mit_sot_new = [
                torch.cat([old_mit_sot[1:], new_val[None, ...]], dim=0)
        ...
        def pytorch_inner_func(carry, x):
            inner_args = pytorch_args_to_inner_func_args(carry, x)
            inner_scan_outs = list(scan_inner_func(*inner_args))
            new_carry, traced_outs = inner_func_outs_to_pytorch_outs(carry, inner_scan_outs)
            return new_carry, traced_outs

        # Extract PyTensor scan outputs
        final_carry, traces = torch.scan(
            pytorch_inner_func, init_carry, seqs, length=n_steps
        )

        def get_partial_traces(traces):
            """Convert PyTorch scan traces to PyTensor traces.

            We need to:
                1. Prepend initial states to PyTorch output traces
            ...
            for init_state, trace, buffer in zip(init_states, traces, buffers):
                if init_state is not None:
                    ...
                    trace = torch.atleast_1d(trace)
                    init_state = torch.unsqueeze(
                        init_state, list(range(trace.ndim - init_state.ndim))
                    )
                    full_trace = torch.cat([init_state, trace], dim=0)
                    buffer_size = buffer.shape[0]
                else:
                    # NIT-SOT: Buffer is just the number of entries that should be returned
                    full_trace = torch.atleast_1d(trace)
                    buffer_size = buffer
