...
def _temme_eta(a, x, const):
    ...
    sigma = (x - a) / a
    return mx.sign(sigma) * mx.sqrt(
        mx.maximum(_log1p_deficit(sigma, x / a, const), const(0.0))
    )
...
static inline float gi_eta(float a, float x) {{
    float sigma = (x - a) / a;
    return sign(sigma) * sqrt(fmax(log1p_deficit(sigma, x / a), 0.0f));
}}
...
static inline float gi_temme(float a, float x, float sgn) {{
    ...
    return 0.5f * erfc_full(sgn * eta * sqrt(a * 0.5f)) + sgn * correction;
}}
"""
)
