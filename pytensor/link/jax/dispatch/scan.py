...
def call_inner_func_with_indexed_buffers(
    ...
    def to_positive_taps(taps):
        # E.g., Convert negative taps (-2, -1) to positive indices (0, 1)
        np_res = np.array(taps) + abs(min(taps))
        return tuple(np_res.tolist())

    sequence_vals = [seq[i] for seq in sequences]
    mit_mot_vals = list(
        chain.from_iterable(
            (
                mit_mot_buffer[((i + tap) % mit_mot_buffer.shape[0])]
                for tap in mit_mot_taps
            )
            for mit_mot_buffer, mit_mot_taps in zip(
    ...
    mit_sot_vals = list(
        chain.from_iterable(
            (
                mit_sot_buffer[((i + j) % mit_sot_buffer.shape[0])]
                for j in to_positive_taps(mit_sot_taps)
            )
            for mit_sot_buffer, mit_sot_taps in zip(
                mit_sot_buffers, info.mit_sot_in_slices, strict=True
            )
        )
    )
    sit_sot_vals = [
        sit_sot_buffer[i % sit_sot_buffer.shape[0]]
        for sit_sot_buffer in sit_sot_buffers
    ]
...
def update_buffers(buffers, update_vals, indices):
    return tuple(
        buffer.at[index % buffer.shape[0]].set(update_val)
...
def align_buffers(buffers, n_steps, max_taps):
    return [
        jnp.roll(
            buffer,
            shift=jnp.where(
                # Only needs rolling if last write position
...
def jax_funcify_Scan(op: Scan, node, **kwargs):
    ...
    def scan(*outer_inputs, op=op, node=node):
        ...
        aligned_mit_mot_buffers = align_buffers(
            updated_mit_mot_buffers,
            n_steps,
            max_taps=[max(taps) for taps in info.mit_mot_out_slices],
        )
