...
class JAXOp(Op):
    ...
    def make_node(self, *inputs: Variable) -> Apply:
        for input, expected in zip(inputs, self.input_types, strict=True):
            if input.type != expected:
                raise TypeError(
                    f"Expected input of type {expected}, got {input.type} instead."
                )
    ...
    def grad(self, inputs, output_gradients):
        ...
        def vjp_jax_op(*args):
            ...
            def func_restricted(*inputs):
                ...
                return [out[i] for i in wrt_index]

            _primals, vjp_fn = jax.vjp(func_restricted, *inputs)
            return vjp_fn(covectors)

        op = JAXOp(
            self.input_types + [self.output_types[i] for i in wrt_index],
            [self.input_types[i] for i in range(num_inputs)],
            vjp_jax_op,
            name="VJP" + (self.name if self.name is not None else ""),
        )

        return op(inputs + [output_gradients[i] for i in wrt_index])
