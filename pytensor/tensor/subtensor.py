...
class AdvancedIncSubtensor(BaseSubtensor, COp):
    ...
    def c_code(self, node, name, input_names, output_names, sub):
        # Only the leading-axis integer-vector update x[vec] (+)= y with 1-D x and
        # 1-D y has a C impl; anything else falls back to `perform`.
        ...
        if (PyArray_NDIM({idx}) != 1) {{
            PyErr_Format(PyExc_ValueError, "AdvancedIncSubtensor: indices ndim should be 1, got %d", PyArray_NDIM({idx}));
            {fail}
        }}
        if (PyArray_NDIM({y}) != 1) {{
            PyErr_Format(PyExc_ValueError, "AdvancedIncSubtensor: second input (y) ndim should be 1, got %d", PyArray_NDIM({y}));
            {fail}
        }}
        if (PyArray_SHAPE({y})[0] != PyArray_SHAPE({idx})[0]) {{
            if ((PyArray_NDIM({y}) == 1) && (PyArray_SHAPE({y})[0] == 1)){{
                ...
                PyErr_Format(PyExc_ValueError,
                "AdvancedIncSubtensor: Shapes of second input (y) and indices do not match: %d, %d",
