...
class QR(Op):
    ...
    def __init__(
        ...
        lwork_cache: tuple[int, int] = (-1, -1),
        pivoting: bool = False,
        check_finite: bool = False,
        last_shape_cache: tuple[tuple[int, int], tuple[int, int]] = ((0, 0), (0, 0)),
    ):
        self.mode = mode
        self.overwrite_a = overwrite_a
        self.lwork_cache = lwork_cache
        ...
        # Used to cache lwork size in the C backend, where possible
        # The idea here is that in vectorized cases (e.g. blockwise), the input shape will not change between perform
        # calls, so we can optimize the LAPACK call to avoid recomputing the optimal lwork size every time.
        self.last_shape_cache = last_shape_cache
    ...
    def make_node(self, x):
        ...
        match self.mode:
            case "full":
                outputs = [
                    ...
                    tensor("R", shape=(M, N), dtype=out_dtype),
                ]
            case "economic":
                outputs = [
                    tensor("Q", shape=(M, K), dtype=out_dtype),
                    tensor("R", shape=(K, N), dtype=out_dtype),
                ]
            case "r":
                outputs = [
                    tensor("R", shape=(M, N), dtype=out_dtype),
            ...
            case "raw":
                outputs = [
                    tensor("H", shape=(M, M), dtype=out_dtype),
                    tensor("tau", shape=(K,), dtype=out_dtype),
                    tensor("R", shape=(M, N), dtype=out_dtype),
                ]
            case _:
                raise NotImplementedError
        ...
        if self.pivoting:
            outputs = [*outputs, tensor("P", shape=(N,), dtype="int32")]
    ...
    def _call_and_get_lwork(self, fn, func_idx, *args, **kwargs):
        last_shape_cache = list(self.last_shape_cache)
        lwork_cache = list(self.lwork_cache)

        last_shape = last_shape_cache[func_idx]
        lwork = lwork_cache[func_idx]

        M, N = args[0].shape

        if lwork in [-1, None] or last_shape != (M, N):
            # If we have no lwork cached, or if the shape of the input has changed, we need to recompute lwork.
            *_, work, info = fn(*args, lwork=-1, **kwargs)

            last_shape_cache[func_idx] = int(work.item())
            lwork_cache[func_idx] = int(work.item())

            self.last_shape_cache = tuple(last_shape_cache)
            self.lwork_cache = tuple(lwork_cache)

        return fn(*args, lwork=self.lwork_cache[func_idx], **kwargs)
    ...
    def perform(self, node, inputs, outputs):
        ...
        if M < N:
            Q, work, info = self._call_and_get_lwork(
                gor_un_gqr, 1, qr[:, :M], tau, overwrite_a=1
            )
        elif self.mode == "economic":
            Q, work, info = self._call_and_get_lwork(
                gor_un_gqr, 1, qr, tau, overwrite_a=1
            )
        else:
            t = qr.dtype.char
            ...
            Q, work, info = self._call_and_get_lwork(
                gor_un_gqr, 1, qqr, tau, overwrite_a=1
            )
