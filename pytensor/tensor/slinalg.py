...
class QZ(Op):
    ...
    __props__ = ("output", "overwrite_a", "overwrite_b", "sort", "return_eigenvalues")
    ...
    def make_node(self, A, B):
        ...
        if self.return_eigenvalues:
            ...
            if complex_input or self.output == "complex":
                alpha_dtype = out_dtype
            else:
                alpha_dtype = "complex64" if A.dtype == "float32" else "complex128"
    ...
    def perform(self, node, inputs, outputs):
        ...
        if self.output == "complex" and not np.iscomplexobj(B):
            overwrite_b = False
        ...
        if self.output == "real" and np.iscomplexobj(B):
            overwrite_b = False
...
def qz(
    ...
    """
    qz_op = QZ(output=output, sort=sort, return_eigenvalues=return_eigenvalues)
