...
class Convolve1d(COp):
    ...
    def make_node(self, in1, in2, is_full_mode):
        ...
        match is_full_mode:
            case Constant():
                static_mode = "full" if is_full_mode.data else "valid"
            case _:
                static_mode = None
        ...
        return Apply(self, [in1, in2, is_full_mode], [out])
    ...
    def perform(self, node, inputs, outputs):
        ...
        in1, in2, is_full_mode = inputs
        outputs[0][0] = numpy_convolve(
            in1, in2, mode="full" if is_full_mode else "valid"
        )
    ...
    def infer_shape(self, fgraph, node, shapes):
        ...
        shape_full = n + k - 1
        shape = switch(is_full_mode, shape_full, shape_valid)
        return [[shape]]
    ...
    def L_op(self, inputs, outputs, output_grads):
        ...
        # The expression below is equivalent to switch(mode | (k >= n), False, True)
        mode_in1_bar = ~is_full_mode & (n > k)
        # If mode is "full", or mode is "valid" and n >= k, then in2_bar mode should use "valid" convolve
        # The expression below is equivalent to switch(mode | (n >= k), False, True)
        mode_in2_bar = ~is_full_mode & (k > n)

        return [
            self(grad, in2[::-1], mode_in1_bar),
            self(grad, in1[::-1], mode_in2_bar),
            DisconnectedType()(),
        ]
    ...
    def c_code(self, node, name, inputs, outputs, sub):
        ...
        {{
            ...
            Py_XDECREF({out});
            {out} = (PyArrayObject*) PyArray_Correlate2((PyObject*){in1}, (PyObject*)in2_flipped_view, {is_full_mode} ? 2 : 0);
...
def convolve1d(
    ...
    is_full_mode = as_scalar(np.bool_(mode == "full"))
    return cast(TensorVariable, blockwise_convolve_1d(in1, in2, is_full_mode))
