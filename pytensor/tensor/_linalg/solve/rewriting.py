...
def split_lu_solve_steps(fgraph, node):
...
@node_rewriter([Scan])
def scan_pushout_solve_lu_decomposition(fgraph, node):
    """This rewrite checks if we have a Solve within a Scan whose A is a function of non-sequences.

    It splits the decomposition of A from the solve with b, so that it can then be pushed out by the
    `scan_pushout_non_sequences_rewrite`
    """
...
scan_seqopt1.register(
    scan_pushout_solve_lu_decomposition.__name__,
    in2out(scan_pushout_solve_lu_decomposition, ignore_newtrees=True),
    "fast_run",
    "scan",
    "scan_pushout",
    position=2,
