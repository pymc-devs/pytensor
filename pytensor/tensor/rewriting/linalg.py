...
@register_specialize
@node_rewriter([det])
def det_of_cholesky(fgraph, node):
    """
    ...
    for cl, _ in fgraph.clients[x]:
        match (cl.op, *cl.outputs):
            case (Blockwise(Cholesky()), L):
                return [prod(diagonal(L, axis1=-2, axis2=-1), axis=-1) ** 2]


@register_stabilize
@register_specialize
@node_rewriter([log])
def log_of_prod_to_sum_of_log(fgraph, node):
    """
    This utilizes a boolean `positive` tag on matrices.
    """
    [x] = node.inputs
    match x.owner_op_and_inputs:
        case (Prod(axis=axis), p):
            ...
            if getattr(p.tag, "positive", False):
                return [log(p).sum(axis=axis)]
...
def useless_consecutive_inv(fgraph, node):
    """
